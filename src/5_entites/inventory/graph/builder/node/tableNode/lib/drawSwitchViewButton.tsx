...
export const drawSwitchViewButton = ({ group, nodeWidth, theme }: IProps) => {
  const IconComponent = EngineeringRoundedIcon;
  const buttonSize = 25;
  const buttonPadding = 5;
  const offsetY = 3;
  const id = '☻';

  const secondButtonPadding = 35;
  ...
  const buttonShape = group.addShape('rect', {
    attrs: {
      x: nodeWidth - TABLE_NODE_COLUMN_HORIZONTAL_PADDING - buttonSize - secondButtonPadding,
      y: offsetY,
      height: buttonSize,
      width: buttonSize,
